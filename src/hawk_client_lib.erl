...
-spec is_post_req(Data :: list()) -> post | get.
%% @doc Определяет тип запроса
is_post_req(Data) ->

	case proplists:get_value(<<"Transport">>, Data) of
        <<"curl">> -> post;
        <<"socket">> -> post;
        _    -> get
    end.
...
send_message_to_pid(Pid, J_data) ->
	case is_process_alive(Pid) of
		true ->
			...
			hawk_client_lib:get_server_message(<<"send_message">>, false, ?OK, false);
		false ->
			hawk_client_lib:get_server_message(<<"send_message">>, ?ERROR_USER_NOT_ONLINE)
	end.
