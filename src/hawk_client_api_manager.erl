...
-module(hawk_client_api_manager).
...
init([]) ->
	...
	supervisor:start_link({local, hawk_client_api_sup}, hawk_client_api_sup, []),
	hawk_client_queue:new(?NAME_QUEUE, ?MAX_QUEUE),
    {ok, Pool} = hawk_client_api_sup:init_worker_pool(?MIN_WORKERS),
...
handle_call(Params, From, #state{worker_count=WCount} = State) ->
	...
	%% 			case hawk_client_queue:insert(?NAME_QUEUE, {From, Params}) of
	%% 				ok ->
	%% 					State;
	%% 				false ->
	%% 					{ok, Pid} = create_worker(),
...
add_to_queue(Pid, #state{worker_count=WCount} = State) ->
	case hawk_client_queue:shift(?NAME_QUEUE) of
		[] ->
			gen_server:cast(Pid, terminate),
			State#state{worker_count=WCount-1};
		{From, Params} ->
...
get_overflow_worker(Count) ->
	if
		...
		true ->
			...
			Size  = hawk_client_queue:size(?NAME_QUEUE),
...
%% @private
create_worker() -> hawk_client_api_sup:create_worker().
