...
-define(SERVER_REPLY, [{from, hawk_client}, {action, <<"">>}, {result, <<"">>}, {error, false}]).
...
-define(get_user_by_key(),
	fun() ->
		Key =
			case hawk_client_app:get_app_env(api_key, undefined) of
			    undefined -> throw("api key not set");
				K -> K
		    end,
